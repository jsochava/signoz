...
func (p *PostablePipeline) IsValid() error {
	...
	if err != nil {
		return errors.Wrapf(err, errors.TypeInvalidInput, errors.CodeInvalidInput, "filter for pipeline %v is not correct", p.Name)
	}
...
func isValidOperator(op PipelineOperator) error {
	if op.ID == "" {
		return errors.NewInvalidInputf(errors.CodeInvalidInput, "PipelineOperator.ID is required")
	}
	...
	case "regex_parser":
		...
		if err != nil {
			return errors.Wrapf(err, errors.TypeInvalidInput, errors.CodeInvalidInput, "error compiling regex expression of %s regex operator", op.ID)
		}
	...
	case "time_parser":
		...
		if op.LayoutType == "epoch" && !slices.Contains(validEpochLayouts, op.Layout) {
			return errors.NewInvalidInputf(errors.CodeInvalidInput, "invalid epoch format '%s' of time parsing processor %s", op.LayoutType, op.ID)
		}
		...
		if op.LayoutType == "strptime" {
			...
			if err != nil {
				return errors.Wrapf(err, errors.TypeInvalidInput, errors.CodeInvalidInput, "invalid strptime format '%s' of time parsing processor %s", op.LayoutType, op.ID)
			}
		}
	...
	default:
		return errors.NewInvalidInputf(errors.CodeInvalidInput, "operator type %s not supported for %s, use one of (grok_parser, regex_parser, copy, move, add, remove, trace_parser, retain)", op.Type, op.ID)
	}
