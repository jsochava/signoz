...
func ValidateTimestamp(timestamp int64, fieldName string) error {
	if timestamp == 0 {
		return errors.Newf(errors.TypeInvalidInput, errors.CodeInvalidInput, "%s is required", fieldName)
	}
	if timestamp < 0 {
		return errors.Newf(errors.TypeInvalidInput, errors.CodeInvalidInput, "%s must be positive", fieldName)
	}
	return nil
}
...
func ValidateFunnelSteps(steps []*FunnelStep) error {
	if len(steps) < 2 {
		return errors.Newf(errors.TypeInvalidInput, errors.CodeInvalidInput, "funnel must have at least 2 steps")
	}

	for i, step := range steps {
		if step.ServiceName == "" {
			return errors.Newf(errors.TypeInvalidInput, errors.CodeInvalidInput, "step %d: service name is required", i+1)
		}
		if step.SpanName == "" {
			return errors.Newf(errors.TypeInvalidInput, errors.CodeInvalidInput, "step %d: span name is required", i+1)
		}
		if step.Order < 0 {
			return errors.Newf(errors.TypeInvalidInput, errors.CodeInvalidInput, "step %d: order must be non-negative", i+1)
		}
	}
