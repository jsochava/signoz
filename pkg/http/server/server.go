...
func New(logger *slog.Logger, cfg Config, handler http.Handler) (*Server, error) {
	if handler == nil {
		return nil, errors.New(errors.TypeInvalidInput, errors.CodeInvalidInput, "cannot build http server, handler is required")
	}

	if logger == nil {
		return nil, errors.New(errors.TypeInvalidInput, errors.CodeInvalidInput, "cannot build http server, logger is required")
	}
