...
func New(ctx context.Context, settings factory.ProviderSettings, config web.Config) (web.Web, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "cannot access web directory")
	}

	ok := fi.IsDir()
	if !ok {
		return nil, errors.Errorf("web directory is not a directory")
	}

	fi, err = os.Stat(filepath.Join(config.Directory, indexFileName))
	if err != nil {
		return nil, errors.Wrap(err, "cannot access "+indexFileName+" in web directory")
	}

	if os.IsNotExist(err) || fi.IsDir() {
		return nil, errors.Errorf("%q does not exist", indexFileName)
	}
...
func (provider *provider) AddToRouter(router *mux.Router) error {
	...
	if err != nil {
		return errors.Wrap(err, "unable to add web to router")
	}
