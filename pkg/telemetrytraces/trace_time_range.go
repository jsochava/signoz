...
func (f *TraceTimeRangeFinder) GetTraceTimeRangeMulti(ctx context.Context, traceIDs []string) (startNano, endNano int64, err error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return 0, 0, errors.NewInvalidInputf(errors.CodeInvalidInput, "traces not found: %v", cleanedIDs)
		}
		// classify as invalid input for consistency with the project's error helpers
		return 0, 0, errors.NewInvalidInputf(errors.CodeInvalidInput, "failed to query trace time range: %v", err)
	}
