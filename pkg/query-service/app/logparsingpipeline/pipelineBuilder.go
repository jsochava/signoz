			...
			if operator.Type == "regex_parser" {
				...
				if err != nil {
					return nil, errors.WrapInvalidInputf(err, CodeFieldNilCheckType,
						"couldn't generate nil check for parseFrom of regex op %s", operator.Name,
					)
				}
				...
				if err != nil {
					return nil, errors.WrapInvalidInputf(err, CodeFieldNilCheckType,
						"couldn't generate nil check for parseFrom of grok op %s", operator.Name,
					)
				}
				...
				if err != nil {
					return nil, errors.WrapInvalidInputf(err, CodeInvalidOperatorType,
						"couldn't process json_parser op %s", operator.Name,
					)
				}
				...
				if strings.HasPrefix(operator.Value, "EXPR(") && strings.HasSuffix(operator.Value, ")") {
					...
					if err != nil {
						return nil, errors.WrapInvalidInputf(err, CodeFieldNilCheckType,
							"could'nt generate nil check for fields referenced in value expr of add operator %s",
							operator.Name,
						)
					}
				...
				if err != nil {
					return nil, errors.WrapInvalidInputf(err, CodeFieldNilCheckType,
						"couldn't generate nil check for From field of %s op %s", operator.Type, operator.Name,
					)
				}
				...
				if err != nil {
					return nil, errors.WrapInvalidInputf(err, CodeFieldNilCheckType,
						"couldn't generate nil check for field to be removed by op %s", operator.Name,
					)
				}
				...
				if err != nil {
					return nil, errors.WrapInvalidInputf(err, CodeFieldNilCheckType,
						"couldn't generate nil check for parseFrom of time parser op %s", operator.Name,
					)
				}
				operator.If = parseFromNotNilCheck
				...
				if operator.LayoutType == "strptime" {
					...
					if err != nil {
						return nil, errors.WrapInvalidInputf(err, CodeFieldNilCheckType,
							"couldn't generate layout regex for time_parser %s", operator.Name,
						)
					}
...
func processJSONParser(parent *pipelinetypes.PipelineOperator) ([]pipelinetypes.PipelineOperator, error) {
	...
	cloneAndReverse := func(input []string) []string {
		...
		cloned = append([]string(nil), cloned...)
	...
	generateMoveOperators := func(keywords []string, to string) error {
		for _, keyword := range cloneAndReverse(keywords) {
			...
			if err != nil {
				return errors.WrapInvalidInputf(err, CodeFieldNilCheckType,
					"couldn't generate nil check for From field of move op in json_parser %s", parent.Name,
				)
			}
	...
	// JSONMapping: host
	if err := generateMoveOperators(mapping[pipelinetypes.Host], `resource["host.name"]`); err != nil {
		return nil, err
	}

	// JSONMapping: service
	if err := generateMoveOperators(mapping[pipelinetypes.Service], `resource["service.name"]`); err != nil {
		return nil, err
	}
	...
	for _, keyword := range cloneAndReverse(mapping[pipelinetypes.Severity]) {
		...
		if err := processSeverityParser(&operator); err != nil {
			return nil, err
		}
	...
	if err := generateMoveOperators(mapping[pipelinetypes.Environment], `resource["deployment.environment.name"]`); err != nil {
		return nil, err
	}

	// JSONMapping: body
	if err := generateMoveOperators(mapping[pipelinetypes.Message], `body`); err != nil {
		return nil, err
	}
...
func fieldsReferencedInExprNotNilCheck(exprStr string) (string, error) {
	referencedFields, err := logFieldsReferencedInExpr(exprStr)
	if err != nil {
		return "", errors.WrapInvalidInputf(err, CodeFieldNilCheckType,
			"couldn't extract log fields referenced in expr %s", exprStr,
		)
	}
	...
	for _, field := range deepestFieldRefs {
		...
		if err != nil {
			return "", errors.WrapInvalidInputf(err, CodeFieldNilCheckType,
				"could not create nil check for %s", field,
			)
		}
...
func logFieldsReferencedInExpr(exprStr string) ([]string, error) {
	// parse abstract syntax tree for expr
	exprAst, err := parser.Parse(exprStr)
	if err != nil {
		return nil, errors.WrapInvalidInputf(err, CodeInvalidOperatorType, "could not parse expr")
	}
