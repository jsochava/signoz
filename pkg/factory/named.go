...
func NewNamedMap[T Named](factories ...T) (NamedMap[T], error) {
	...
	for _, factory := range factories {
		if _, ok := fmap[factory.Name()]; ok {
			return NamedMap[T]{}, errors.New(
				errors.TypeAlreadyExists,
				errors.CodeAlreadyExists,
				fmt.Sprintf("cannot build factory map, duplicate name %q found", factory.Name()),
			)
		}
...
func (n *NamedMap[T]) Get(namestr string) (t T, err error) {
	...
	if !ok {
		err = errors.New(
			errors.TypeNotFound,
			errors.CodeNotFound,
			fmt.Sprintf("factory %q not found or not registered", name),
		)
		return
	}
...
func (n *NamedMap[T]) Add(factory T) (err error) {
	...
	if _, ok := n.factories[name]; ok {
		return errors.New(
			errors.TypeAlreadyExists,
			errors.CodeAlreadyExists,
			fmt.Sprintf("factory %q already exists", name),
		)
	}
